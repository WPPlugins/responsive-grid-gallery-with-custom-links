...
function abcfrggcl_grid( $scodeArgs ){
    ...
    $centerCls = abcfrggcl_util_center_cls( $gCntrCenter, $clsPfix );
    $cntrStyle = abcfl_css_w_responsive( $gCntrW, $gCntrW ) . $gCntrStyle;

    //GRID CONTAINER classes
    $gCntrCustomCls = $gCntrCls . $centerCls . ' ' . $clsPfix . 'v_' . $pversion;;
    $div = abcfrggcl_util_generic_div( $clsPfix, 'GridCntr', $gCntrCustomCls, $cntrStyle, $clsPfix . 'GridCntr_' . $tplateID );

    //Grid items
    $gridItems = abcfrggcl_grid_items( $tplateID, $tplateOptns, $clsPfix, $vAid );
...
function abcfrggcl_grid_items( $tplateID, $tplateOptns, $clsPfix, $vAid ){
    ...
    if ( $postIDs ) {
        foreach ( $postIDs as $itemID ) {
            ...
            $itemsHTML .= abcfrggcl_grid_item( $itemID, $tplateOptns, $itemMarginLR, $itemMarginB, $itemCls, $itemStyle, $clsPfix, $addMaxW, $vAid );
...
function abcfrggcl_grid_item( $itemID, $tplateOptns, $itemMarginLR, $itemMarginB, $itemCls, $itemStyle, $clsPfix, $addMaxW, $vAid ){

    $colsQty = isset( $tplateOptns['_gridCols'] ) ? esc_attr( $tplateOptns['_gridCols'][0] ) : '2';

    //DIV - Item container.
    $div = abcfrggcl_grid_item_cntr_div( $colsQty, $itemMarginLR, $itemMarginB, $itemCls, $itemStyle, $clsPfix, $vAid);
    $itemOptns = get_post_custom( $itemID );

    $imgCntr = abcfrggcl_grid_items_image_cntr( $tplateOptns, $itemOptns, $clsPfix );
    $txtCntr = abcfrggcl_grid_item_txt_cntr( $itemOptns, $tplateOptns, $clsPfix, $vAid, $addMaxW );

    return $div['itemCntrS'] . $imgCntr . $txtCntr . $div['itemCntrE'];
}

//ITEM CONTAINER ( empty )
function abcfrggcl_grid_item_cntr_div( $colsQty, $itemMarginLR, $itemMarginB, $itemCls, $itemStyle, $clsPfix, $vAid ){
    ...
    $columnsCls =  $clsPfix . 'GridCol ' . $clsPfix . 'GridCol_' . $colsQty;

    //LR and Bottom margins.
    $marginsCls = rtrim(' ' . abcfrggcl_util_cls_name_nc_bldr( $itemMarginLR, 'PadLR', $clsPfix, '1' ) . ' ' . abcfrggcl_util_cls_name_nc_bldr( $itemMarginB, 'MB', $clsPfix, '40' ));
...
function abcfrggcl_grid_item_txt_cntr( $itemOptns, $tplateOptns, $clsPfix, $vAid, $addMaxW ){
    ...
    if($addMaxW == 'Y'){
        ...
        $maxWCntr = abcfrggcl_grid_item_txt_cntr_max_w( $imgID, $imgUrl, $addMaxW, $clsPfix );
    }

    $vaAidCls = '';
    if($vAid == 'Y') { $vaAidCls = 'VAidTxt'; }
    $div = abcfrggcl_util_generic_div( $clsPfix, 'GridTxtCntr', $txtCntrCls, $txtCntrStyle, '', $vaAidCls );

    $itemLinesHTML = abcfrggcl_grid_items_txt_field ( $itemOptns, $tplateOptns, 'F1', $clsPfix );
