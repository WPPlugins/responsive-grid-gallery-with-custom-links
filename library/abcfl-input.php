...
function abcfl_input_sec_icon_hdr_hlp( $imgsQURL, $iconName, $txt1, $txt2, $hlpHref, $clsCntrCust='', $clsTxt1Cust='', $clsTxt2Cust='', $target='_blank' ){

    $clsTxt1 = 'abcflFontWPHdr abcflFontS16 abcflFontW600';
    ...
    if( !empty( $hlpHref ) ){
        $divTxt1 = abcfl_input_sec_title_hlp( $imgsQURL, $txt1, $hlpHref, $clsTxt1, $target );
    }

    $icon = abcfl_html_img_tag( '', $imgsQURL . $iconName, '', '' );
...
//Section Name & ? icon. Custom class overwrites all default classes.
if ( !function_exists( 'abcfl_input_sec_title_hlp' ) ){
    function abcfl_input_sec_title_hlp( $imgsQURL, $txt, $hlpUrl, $clsCust='', $target='_blank' ) {

        //No link. Label only.
        if( empty( $hlpUrl ) ) { return abcfl_input_sec_title( $txt, $clsCust ); }

        $cls = 'abcflFontWPHdr abcflFontS16 abcflFontW600 abcflMTop5';
        if(!empty($clsCust)){ $cls = trim($clsCust); }

        $hlpIcon = abcfl_html_img_tag( '', $imgsQURL . 'help.png', 'Help', 'Help', 40, 24, 'abcflVABottom' );
        $hlpURL = abcfl_html_a_tag( $hlpUrl, $hlpIcon, $target );
...
if ( !function_exists( 'abcfl_input_hlp_url' ) ){
    function abcfl_input_hlp_url( $txt, $hlpUrl, $clsCust='', $target='_blank' ) {

        $clsLbl = 'abcflFontFVS14 abcflFontW400 abcflMTop10';
        if(!empty( $clsCust ) ){ $clsLbl = $clsCust; }

        $hlpURL = abcfl_html_a_tag( $hlpUrl, $txt, $target );
...
if ( !function_exists( 'abcfl_input_sec_title' ) ){
    function abcfl_input_sec_title( $txt, $clsCust='' ) {
        ...
        $cls = 'abcflFontWPHdr abcflFontS16 abcflFontW600 abcflMTop10';
        if(!empty($clsCust)){ $cls = trim($clsCust); }
...
if ( !function_exists( 'abcfl_input_info_lbl' ) ){
    function abcfl_input_info_lbl($txt, $cls='', $fontS='12', $fontW='N') {
        ...
        return '<div class="abcflFontWPHdr' . $clsLbl . '">' . $txt . '</div>';
    }
}
